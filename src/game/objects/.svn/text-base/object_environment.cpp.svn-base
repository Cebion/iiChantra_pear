...
ObjEnvironment* CreateEnvironment(const char* proto_name, Vector2 coord)
{
	...
	env->aabb.p = coord;
	//env->sprite = NULL; <- ѕлоха€ строчка, плоха€! ¬ызывает утечку! ѕочему...?
...
void ObjEnvironment::OnEnter( GameObject* obj )
{
	...
	//»щем точку дл€ передачи параметром
	Vector2 pnt = obj->aabb.p;
	//—начала ищем возможное пересечение внутри
	...
	if ( obj->aabb.Right() > right && right > obj->aabb.Left() && left < obj->aabb.Left() )
		...
		pnt.y = left;
	//¬нутренних пересечений не было, отслеживаем скорость назад.
...
void ObjEnvironment::OnLeave( GameObject* obj )
{
	...
	ObjDynamic* eob = (ObjDynamic*)obj;
	//»щем точку дл€ передачи параметром, пересечени€ внутри точно нет.
...
void DefaultEnvSet( ObjEnvironment* env )
{
	if ( env )
	{
		...
		//memcpy(default_environment, env, sizeof(ObjEnvironment)); <= не сработает, потому что строки, на которые ссылаютс€ списки могут быть удалены со старым окружением.
	}
	else 
	{
		DefaultEnvDelete();
